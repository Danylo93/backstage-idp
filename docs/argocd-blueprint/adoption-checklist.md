...
kubectl apply -f backend-apps-manual-hmg.yaml
...
Para migrar sem parar o fluxo:

1. Comece usando `dev/prd` com os novos arquivos (`hmg/hub` em paralelo).
2. Mantenha `rc/stg` como alias temporario, ou ajuste a function para escrever `hmg/hub`.
3. Depois da migracao, remova os aliases.
